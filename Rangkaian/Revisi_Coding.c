...
const unsigned long feedInterval = 20000;
...
void uart_print_float_labeled(const char *label, float value, uint8_t decimal) {
    char buffer[40];
    char value_str[16];

    dtostrf(value, 1, decimal, value_str);  // konversi float ke string
    sprintf(buffer, "%s%s\r\n", label, value_str);
    uart_print(buffer);
}
...
uint8_t button_pressed() {
    return (PIND & (1 << BUTTON_PIN)); // aktif saat HIGH
}
...
float regPh = 0.0;
uint8_t statePh = 0;  // FSM state untuk pembacaan pH

float konversi_ph(int adc) {
    float TeganganPh = (VREF / 1024.0) * adc;
...
int main(void) {
    DDRD &= ~(1 << BUTTON_PIN);
    PORTD &= ~(1 << BUTTON_PIN); // Nonaktifkan pull-up karena tombol ke 5V
    DDRD |= (1 << LED_PIN);
    PORTD &= ~(1 << LED_PIN);
    ...
    while (1) {
        ...
        if (feedState == IDLE) {
            ...
            // Kirim string timer ke serial dalam format Timer: HH:MM:SS
            char timerStr[16];
            sprintf(timerStr, "Timer: %02u:%02u:%02u", hours, minutes, seconds);
            uart_println(timerStr);

        } else {
            lcd_print("Feeding...       ");
            // Saat feeding bisa kirim status khusus jika perlu
            uart_println("Timer: Feeding");
        }

// FSM pH
switch (statePh) {
    case 0: // Baca dan akumulasi
        {
            uint16_t adcPh = adc_read(0); // Ganti 0 dengan channel ADC yang kamu pakai
            totalPh += adcPh;
            sampleCount++;

            if (sampleCount >= JUMLAH_SAMPEL) {
                statePh = 1; // Pindah ke proses pH
            }
        }
        break;

    case 1: // Proses rata-rata dan konversi
        {
            int rata2Ph = totalPh / JUMLAH_SAMPEL;
            uart_print_int("Nilai rata-rata ADC Ph: ", rata2Ph);

            regPh = konversi_ph(rata2Ph);
            uart_print_float_labeled("Nilai regPh: ", regPh, 3);  // ✅ BENAR
    ...
    case 2: // Delay atau idle
        _delay_ms(500);  // Delay antar pembacaan
        statePh = 0;
        break;
}
